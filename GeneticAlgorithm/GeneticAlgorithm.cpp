...
Ví dụ với mã hóa nhị phân, ta lấy ngẫu nhiên một số thực [0, 0.5) để biểu thị 0 và một số thực ngẫu nhiên [0.5, 1] để biểu thị 1
    ...
    +---+----------------------------------+
Với kiểu mã hóa này, thì ta cần một hàm giải mã decode(vector<double> chormosome)

Khởi tạo quần thể -> vector<vector<Individual>> populutation

Các phương pháp chọn lọc cá thể cha mẹ
- Chọn lọc ngẫu nhiên - random selection - RS
    Mô tả: Lấy ngẫu nhiên một số lượng cá thể trong tập quần thể - population để bổ sung vào tập cha mẹ - listParent
